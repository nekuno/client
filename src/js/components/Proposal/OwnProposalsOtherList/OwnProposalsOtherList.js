...
export default class OwnProposalsOtherList extends Component {
    ...
    clickProposal(proposalLiked) {
        const proposalId = proposalLiked.proposal.id;
        const userSlug = proposalLiked.owner.slug;
    ...
    getProposalSummaries() {
        ...
        let summaries = proposalsLiked.map((proposalLiked) => {
            ...
            return <div key={proposal.id} onClick={this.clickProposal.bind(this, proposalLiked)}><ProposalSummary proposal={proposal} hasMatch={hasMatch} owner={owner} onClickHandler={this.clickProposal}/></div>
        });
