...
import FrameCollapsible from "../../../components/ui/FrameCollapsible";
...
export default class ProposalFeaturesPage extends Component {
	...
	constructor(props) {
		...
		const data =
			props.thread &&
			props.thread.filters &&
			props.thread.filters.userFilters
				? props.thread.filters.userFilters
				: {};

		this.state = {
			data: CreatingProposalStore.proposal.filters.userFilters
				? CreatingProposalStore.proposal.filters.userFilters
				: data
		};
		...
		this.handleChangeFilterAndUnSelect = this.handleChangeFilterAndUnSelect.bind(
			this
		);
	...
	componentDidUpdate() {
		...
		if (
			...
			thread &&
			thread.filters &&
			thread.filters.userFilters &&
			Object.keys(thread.filters.userFilters).length > 0
		) {
	...
	handleClickRemoveFilter(field) {
		let userFilters = Object.assign(
			{},
			this.props.thread.filters.userFilters
		);
		...
		if (CreatingProposalStore.proposal.type === "work") {
			...
			if (categories) {
				categories.forEach(function(category, categoryIndex) {
					categoriesToHide.forEach(function(
						categoryToHide,
						categoryToHideIndex
					) {
						if (
							JSON.stringify(category.fields) ===
							JSON.stringify(categoryToHide)
						) {
		...
		if (
			...
			switch (filter.type) {
				case "location_distance":
					...
					break;
				case "integer_range":
					return this.renderIntegerRangeFilter(
						field,
						filter,
						filterData
					);
				...
				case "multiple_choices":
					return this.renderMultipleChoicesFilter(
						field,
						filter,
						filterData
					);
				...
				case "double_multiple_choices":
					return this.renderDoubleMultipleChoicesFilter(
						field,
						filter,
						filterData
					);
				...
				case "choice_and_multiple_choices":
					return this.renderChoiceAndMultipleChoicesFilter(
						field,
						filter,
						filterData
					);
				...
				case "tags_and_multiple_choices":
					return this.renderTagsAndMultipleChoicesFilter(
						field,
						filter,
						filterData,
						tags
	...
	renderChoiceAndMultipleChoicesFilter(field, filter, data) {
		return (
			<ChoiceAndMultipleChoicesFilter
				filterKey={field}
				filter={filter}
				data={data}
				handleChangeFilter={this.handleChangeFilter}
	...
	handleStepsBarClick() {
		...
		ProposalActionCreators.mergeCreatingProposal(
...
ProposalFeaturesPage.defaultProps = {
	strings: {
		...
		filterWarning:
			"This filters only be visible for you and we need to filter users",
